...
class tracker():
    ...
    def create_video(self, fileName=None, v_id=None, display_metrics=True, fps=30):
        if display_metrics == True:
            video_utils.create_video(self.all_comp_grids, ratsnest=self.ratsnest, v_id=v_id, fileName=fileName, all_metrics=self.metrics, draw_debug=True, fps=fps)
        else:
            video_utils.create_video(self.all_comp_grids, ratsnest=self.ratsnest, v_id=v_id, fileName=fileName, all_metrics=None, draw_debug=True, fps=fps)
    ...
    def log_run_to_file(self, path=None, filename=None, kicad_pcb=None):
        ...
        for i in range(len(self.metrics[0])):   # iterate over the number of components 
            ...
            f.write("\tdata begin\r\n")
            for j in range(len(self.metrics)-1):    # -1 due to done exiting after the first encounter.
    ...
    def capture_snapshot(self, fileName):
        video_utils.create_image(self.all_comp_grids, ratsnest=self.ratsnest, fileName=fileName, draw_debug=True)

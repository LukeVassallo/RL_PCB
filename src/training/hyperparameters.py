...
def gen_default_hyperparameters(on_policy=False):
    ...
    if on_policy ==  True:
        default_hyperparameters["net_arch"] = [dict(pi=[32, 32, 128, 64, 64], vf=[64, 128, 64])]
...
def sample_hyperparameters(trial: optuna.Trial, on_policy=False) -> Dict[str, Any]:
    ...
    for i in range(pi_layers):
        pi_n_units_l.append(trial.suggest_int("pi_n_units_l{}".format(i),hp_opt_pi_neurons_min,hp_opt_pi_neurons_max))
    
    if on_policy == True: 
        vf_layers = trial.suggest_int("vf_layers", hp_opt_vf_layers_min, hp_opt_vf_layers_max)
        vf_n_units_l = []
        for i in range(vf_layers):
            vf_n_units_l.append(trial.suggest_int("vf_n_units_l{}".format(i),hp_opt_vf_neurons_min,hp_opt_vf_neurons_max))
            
        net_arch = [{"pi": pi_n_units_l, "vf": vf_n_units_l}]
    else: # off_policy
        qf_layers = trial.suggest_int("qf_layers", hp_opt_qf_layers_min, hp_opt_qf_layers_max)
        qf_n_units_l = []
        for i in range(qf_layers):
            qf_n_units_l.append(trial.suggest_int("qf_n_units_l{}".format(i),hp_opt_qf_neurons_min,hp_opt_qf_neurons_max))
...
def sample_hyperparameters_nas( trial: optuna.Trial, algo:str, max_steps:int ) -> Dict[str, Any]:
    pi_layers = trial.suggest_int("pi_layers", hp_opt_pi_layers_min, hp_opt_pi_layers_max)
    pi_n_units_l = []
    for i in range(pi_layers):
        pi_n_units_l.append(trial.suggest_int("pi_n_units_l{}".format(i),hp_opt_pi_neurons_min,hp_opt_pi_neurons_max))
...
def sample_hyperparameters_hp( trial: optuna.Trial, algo:str, max_steps:int, base_hyperparameters:str=None ) -> Dict[str, Any]: 
    ...
    if (algo == "TRPO") or (algo == "PPO"):
        ...
        learning_rate = trial.suggest_float("learning_rate", 1e-5, 1e-2, log=True)
        buffer_size = trial.suggest_int("buffer_size", 300_000, 3_000_000, step=300_000)
        batch_size = trial.suggest_categorical("batch_size", [32, 64, 128, 256, 512, 1024])
        train_freq = trial.suggest_categorical("train_freq", [1, 2, 4, 8, 16, 32, 64, 128, 256])                # trains after every step
...
def save_best_hyperparameters(filename: str, study:optuna.Study, on_policy: bool = False):
    ...
    save_hyperparameters_to_file(filename=filename, hyperparameters=hyperparameters)
...
def hyperparmeters_on_policy(hyperparameters):
    ...
    :return: True if hyperparameters correspond to an on-policy algorithm, false otherwise
    :rtype: bool
...
def hyperparmeters_off_policy(hyperparameters):
    ...
    :param hyperparameters: hyperparameters dictionary
    :type hyperparameters: dictionary
    :return: True if hyperparameters correspond to an off-policy algorithm, false otherwise
    :rtype: bool

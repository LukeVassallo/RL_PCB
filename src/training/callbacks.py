...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 10 18:06:36 2022
...
class log_and_eval_callback():
    def __init__(
        ...
        # Create directory if it doesn't exsit.
        if os.path.isdir(self.video_path) == False:
            os.makedirs(self.video_path)   
            
        # Create directory if it doesn't exsit.
        if os.path.isdir(self.video_train_path) == False:
            os.makedirs(self.video_train_path) 
            
        # Create directory if it doesn't exsit.
        if os.path.isdir(self.video_eval_path) == False:
    ...
    def on_step(self):
        if self.model.done:
            ...
            if episode_reward > self.best_episode_reward:
                ...
                self.model.save(filename=os.path.join(self.model_path,'best'))    
            ...
            if mean_episode_reward > self.best_mean_episode_reward:
                ...
                self.model.save(filename=os.path.join(self.model_path,'best_mean'))   
            
            if (self.model.num_timesteps - self.last_best_mean_timestep) > self.model.early_stopping:
                self.model.exit = True
                            
            self.writer.add_scalar(tag='rollout/mean_episode_length', scalar_value=mean_episode_length, global_step=self.model.num_timesteps)
            self.writer.add_scalar(tag='rollout/mean_episode_reward', scalar_value=mean_episode_reward, global_step=self.model.num_timesteps)
            self.writer.add_scalar(tag='rollout/fps', scalar_value=mean_fps, global_step=self.model.num_timesteps)
            
            mean_actor_loss = np.round(np.mean(self.model.trackr.critic_losses),2)
            mean_critic_loss = np.round(np.mean(self.model.trackr.actor_losses),2)
            
            self.writer.add_scalar(tag='training/mean_actor_loss', scalar_value=mean_actor_loss, global_step=self.model.num_timesteps)
            self.writer.add_scalar(tag='training/mean_critic_loss', scalar_value=mean_critic_loss, global_step=self.model.num_timesteps)

            if self.rl_model_type == "SAC":
                mean_critic_1_loss = np.round(np.mean(self.model.trackr.critic_1_loss),2)
                mean_critic_2_loss = np.round(np.mean(self.model.trackr.critic_2_loss),2)
                mean_entropy_loss = np.round(np.mean(self.model.trackr.entropy_loss),2)
                entropy = np.round(self.model.trackr.entropy[-1],2)
                
                self.writer.add_scalar(tag='training/mean_critic_1_loss', scalar_value=mean_critic_1_loss, global_step=self.model.num_timesteps)
                self.writer.add_scalar(tag='training/mean_critic_2_loss', scalar_value=mean_critic_2_loss, global_step=self.model.num_timesteps)
                self.writer.add_scalar(tag='training/mean_entropy_loss', scalar_value=mean_entropy_loss, global_step=self.model.num_timesteps)
                self.writer.add_scalar(tag='training/entropy', scalar_value=entropy, global_step=self.model.num_timesteps)
        ...
        if self.model.num_timesteps % self.eval_freq == 0:
            ...
            self.writer.add_scalar(tag='periodic_evals/training_dataset/episode_reward', scalar_value=info[0], global_step=self.model.num_timesteps)
            self.writer.add_scalar(tag='periodic_evals/training_dataset/episode_length', scalar_value=info[1], global_step=self.model.num_timesteps)
            if self.verbose:
                mean_episode_length = np.int32(np.round(np.mean(self.model.trackr.episode_length),0))
                mean_episode_reward = np.round(np.mean(self.model.trackr.episode_reward),2)
            ...
            info = self.evaluate(model=None, tag=f'periodic_evals/evaluation_dataset/{int(self.model.num_timesteps/1000)}k', training_dataset=False, quick_eval=True)
            self.writer.add_scalar(tag='periodic_evals/testing_dataset/episode_reward', scalar_value=info[0], global_step=self.model.num_timesteps)
            self.writer.add_scalar(tag='periodic_evals/testing_dataset/episode_length', scalar_value=info[1], global_step=self.model.num_timesteps)
            if self.verbose:
                mean_episode_length = np.int32(np.round(np.mean(self.model.trackr.episode_length),0))
                mean_episode_reward = np.round(np.mean(self.model.trackr.episode_reward),2)
    ...
    def on_training_start(self):
        ...
        if self.training_log is not None:
            self.training_log = open(self.training_log, 'w')
        ...
        self.log_settings(self.settings, initial_We=self.initial_We, hyperparameters=self.hyperparameters, model=self.model, global_step=0)
    ...
    def on_training_end(self):
        ...
        self.best_mean_metrics = self.evaluate(model=os.path.join(self.model_path,'best_mean'), tag='final_evals/training_dataset', training_dataset=False, quick_eval=False, periodic=False, long=True)
    ...
    def evaluate( self, 
        ...
        for i in range(self.num_evaluations):
            ...
            if periodic == True:
                run_output_dir = os.path.join(run_output_dir, f'{int(self.model.num_timesteps/1000)}k/run_{i}')
            else:
                run_output_dir = os.path.join(run_output_dir, f'final/run_{i}')
            
            if os.path.isdir(run_output_dir) == False:
                os.makedirs(run_output_dir) 
            
            evaluation_log = open(os.path.join(run_output_dir,"evaluation.log"), "w")
            ...
            while not done:
                ...
                if self.rl_model_type == "TD3":
                    obs_vec = eval_env.step(model=self.model.actor, random=False, deterministic=True, rl_model_type=self.rl_model_type)
                else:
                    obs_vec = eval_env.step(model=self.model.policy, random=False, deterministic=True, rl_model_type=self.rl_model_type)
                step_reward=0
                ...
                if quick_eval == False: # save best layouts, log video
                    ...
                    if (hpwl < best_hpwl) and (np.max(all_ol) < 1):
                        ...
                        eval_env.write_current_pcb_file(path=run_output_dir, filename=file_best_hpwl_zero_overlap+".pcb")  # overwrite best; unique filename for easier processing with automated tools
                        if verbose == 1:

...
def compute_pad_referenced_distance_vectors_v2(n, nn, e, ignore_power=False):
    ...
    for vecs in all_vecs:
            ...
            for v in vecs:
                v_pts.append(polar_to_rectangular(v[-2]/len(pts),v[-1])) # Divide the magnitude of a vector the sum of vectors in the list.
                #v_pts.append(polar_to_rectangular(v[-2],v[-1])) # Divide the magnitude of a vector the sum of vectors in the list.
...
# computes the sum of shortest distances between the current node and it's neighbors
            ...
            if (e.get_instance_id(0) == current_node_id or e.get_instance_id(0) == neighbor_node_id) and (e.get_instance_id(1) == current_node_id or e.get_instance_id(1) == neighbor_node_id):
                for i in range(2):
                    if e.get_instance_id(i) == current_node_id:
                        ...
                        rotated_current_pad_pos = kicad_rotate(float(current_pad_pos[0]),float(current_pad_pos[1]), current_node_orientation)
...
# computes the sum of shortest distances between the current node's pads and it's neighbors
# This is achieved by first computing the euclidean distance of all edges. These represent a pad to pad connection.
# For each pad of the current node the shortest pad-pad distance to any of its neighbors is noted.
# The sum is returned.
def compute_sum_of_euclidean_distances_between_pads(n, nn, eoi, ignore_power=False):
    current_node_id = n.get_id()
    current_node_pos = n.get_pos()
    current_node_orientation = n.get_orientation()
    ...
    for i in range(current_node_pins):
        lengths = []
        for e in eoi:
            if ignore_power == True and e.get_power_rail() > 0:
                continue
        ...
        #print(f'lengths array {lengths}')
    #if len(all_lengths) != 0: print(f'The contents of all_lengths {all_lengths} and the smallest value is {all_lengths[-1]}')

    # print(n.get_name(), all_lengths)
...
def shortest_distance_to_object_within_segment(img, los_segment, centre, radius, normalize=True, padding=(0,0)):
    ...
    for i in range(tmp.shape[1]):
        for j in range(tmp.shape[0]):
            if tmp[i][j] == 1:
                d = distance_between_two_points((centre[0]+padding[0],centre[1]+padding[1]),(j,i))
                if d < dist:
                    dist = d
                    coords = tuple([j,i])
...
def get_los_feature_vector(n, nn, eoi, b, clamp_at_zero=True, padding=None):
    ...
    los_segments, los_pixels =  draw_los(current_node_position[0],
    ...
    grid_comps = draw_comps_from_nodes_and_edges(n, nn, eoi, b, padding=padding)
    img = grid_comps[0]+grid_comps[1]
    ...
    #res = grid_comps[0] / 64 * los_segments[i]
    los_feature = []
    box_edge_coords = []
    intersection_point_coords = []
    los_radius = int( (np.max(current_node_size)*2) / pcbDraw_resolution())
    for i in range(8):
        # The center point of the current node must factor in the padding of the grid!
        d, c = shortest_distance_to_object_within_segment(grid_comps[0], 
                                                          los_segments[i], 
        ...
        if d < los_radius:     # if distance is less than the radius of the los signal
            if d == 0: 
                ...
                box_edge_coords.append( (scaled_current_node_pos[0], scaled_current_node_pos[1]) )  # component center as edge coord
            else:
                ...
                m = distance_from_rectangle_center_to_edge( (x,y), angle-deg2rad(current_node_orientation), degrees=False )
                
                # p = [0,0]
                # p[0] = np.int0(scaled_current_node_pos[0] + m*np.cos(angle))
                # p[1] = np.int0(scaled_current_node_pos[1] - m*np.sin(angle))
            ...
            angle = 0
            box_edge_coords.append((scaled_current_node_pos[0], scaled_current_node_pos[1]))
            
        if clamp_at_zero == True:
            if edge_to_intersection_dist < 0: edge_to_intersection_dist = 0
...
def cosine_distance_for_two_terminal_component(resultant, degrees=False):
    ...
    degrees - When true the angles are interpreted in degrees. By default they are interpreted in radians.
    """
    #print(rad2deg(a-b))

    if len(resultant) == 2:
        #print(resultant, resultant[0][1][-1], resultant[1][1][-1])
        ...
        #print(resultant, resultant[0][1][-1])
        return 0

...
#sys.path.append('../')  # This path should point to the directory containing the multi-agent files (i.e. core). Otherwise the following three imports will fail
...
def do_work(in_queue, out_list):
    while True:
        ...
        # fake work
        result = evaluation_run(settings=item[0])
...
def evaluation_run(settings):
    ...
    model = setup_model( model_type=settings["policy"], train_env=None, hyperparameters=hp, device=settings["device"] )
    try:
        model.load(settings["model"])
    except:
        print('Failed to load model. Does the hyperparameters file correspond to the model file?')
        sys.exit()
    ...
    # if os.path.isdir(settings["output"]) == True:
    #     print(F'Evaluation output directory \'{settings["output"]}\' exists. Program terminated.')
    #     sys.exit()
    # else:
    ...
    for j in range(get_pcb_num(settings["pcb_file"])):
        ...
        # all_best_hpwl = []
        # all_best_hpwl_at_10_overlap = []
        # all_best_hpwl_at_20_overlap = []
        ...
        eval_env.reset()
        eval_env.write_current_pcb_file(path=sa_dir, filename=current_pcb_filename+f"_{settings['run']}.pcb")
        done = False
        episode_steps=0
        while not done:
            episode_steps += 1
            if settings["policy"] == "TD3": 
                obs_vec = eval_env.step(model=model.actor, random=False, deterministic=True, rl_model_type="TD3")
                step_reward=0
            else:   # SAC 
                obs_vec = eval_env.step(model=model.policy, random=False, deterministic=True, rl_model_type="SAC")
                step_reward=0
        ...
        # if best_hpwl < 1e6:
        #     all_best_hpwl.append(best_hpwl)
        # if best_hpwl_at_10_overlap < 1e6:
        #     all_best_hpwl_at_10_overlap.append(best_hpwl_at_10_overlap)
        # if best_hpwl_at_20_overlap < 1e6:
        #     all_best_hpwl_at_20_overlap.append(best_hpwl_at_20_overlap)
        
    #     all_runs = open(os.path.join(pcb_dir,"all_rl_runs.hpwl"), "a")
    #     all_runs.write(f'{settings["run"]},{best_hpwl},{best_hpwl_at_10_overlap},{best_hpwl_at_20_overlap}\r\n')
    #     all_runs.close()

    # all_runs = open(os.path.join(pcb_dir,"all_rl_runs.hpwl"), "a")
    # all_runs.write(f'mean,{np.mean(all_best_hpwl)},{np.mean(all_best_hpwl_at_10_overlap)},{np.mean(all_best_hpwl_at_20_overlap)}\r\n')
    # all_runs.write(f'valid,{len(all_best_hpwl)}/{settings["runs"]},{len(all_best_hpwl_at_10_overlap)}/{settings["runs"]},{len(all_best_hpwl_at_20_overlap)}/{settings["runs"]}\r\n')
    # all_runs.close()            

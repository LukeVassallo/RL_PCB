...
#sys.path.append("../../final_results")  # Required for setup info contained developed for the single agent experiments.
        ...
        while True:
            ...
            if not line:
                ...
                if fields[0] not in pcbs:
                    ...
                    data[f"{fields[0]}"] = dict()
                
                if f"{fields[1]}" not in data[f"{fields[0]}"]:
                    data[f"{fields[0]}"][f"{fields[1]}"] = dict()
        ...
        for wl_idx in [0, 2]:
            ...
            for pcb_key, pcb_value in data.items():
                ...
                for trial_key, trial_value in pcb_value.items():
                    ...
                    if ("SA_PCB" in trial_value) and (args.skip_simulated_annealing == False):                
                ...
                mean_best_hpwl_00.append(f'{np.round(np.mean( [ x for x in best_hpwl_00[-n_trials:] if type(x) == np.float64 ] ),2)} \u00B1 {np.round(np.std( [ x for x in best_hpwl_00[-n_trials:] if type(x) == np.float64 ] ),2)} ({len([ x for x in best_hpwl_00[-n_trials:] if type(x) == np.float64 ])})')
                ...
                if args.skip_simulated_annealing == False:
                ...
                #mean_best_hpwl_10.append(np.round(np.mean( [ x for x in best_hpwl_10[-n_trials:] if type(x) == np.float64 ] ),2))
                #mean_best_hpwl_20.append(np.round(np.mean( [ x for x in best_hpwl_20[-n_trials:] if type(x) == np.float64 ] ),2))
                #mean_sa_pcb.append(np.round(np.mean( [ x for x in sa_pcb[-n_trials:] if type(x) == np.float64 ] ),2))           
            
            colwidths =[]
            
            if args.skip_simulated_annealing == False:
                cols = (pcb_col, trial_col,  best_hpwl_00,  best_hpwl_10, best_hpwl_20, sa_pcb)
                mean_cols = (mean_pcb_col, mean_trial_col,  mean_best_hpwl_00,  mean_best_hpwl_10, mean_best_hpwl_20, mean_sa_pcb)
                ...
                cols = (pcb_col, trial_col,  best_hpwl_00,  best_hpwl_10, best_hpwl_20)
                mean_cols = (mean_pcb_col, mean_trial_col,  mean_best_hpwl_00,  mean_best_hpwl_10, mean_best_hpwl_20)
            ...
            if wl_idx == 0: 
                #report_data.append(PageBreak())
                report_data.append(Paragraph('Estimated Wirelength (HPWL)',styleH2))        
            else:
                report_data.append(Paragraph('Routed Wirelength',styleH2))        
            
            #report_data.append(Paragraph('<br /><br />',style))
            #report_data.append(Paragraph(f'{settings["experiments"][j]}',style))
            ...
            #report_data.append(Paragraph(f'run involved = {settings["run_dirs"][j]}',style))   
